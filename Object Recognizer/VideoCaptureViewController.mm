...
@synthesize qualityPreset = __qualityPreset;
...
{
    dispatch_queue_t captureQ = dispatch_queue_create("Capture Queue", NULL);
    dispatch_async(captureQ, ^{[self destroyCaptureSession];});
    dispatch_release(captureQ);
...
{    
    ...
    if (format == kCVPixelFormatType_420YpCbCr8BiPlanarFullRange && self.isViewLoaded && self.view.window) {
    ...
    else if (format == kCVPixelFormatType_32BGRA && self.isViewLoaded && self.view.window) {
